...
class ObligationContract : Contract {
    ...
    private fun verifyDefault(tx: LedgerTransaction, signers: Set<PublicKey>) = requireThat {
        ...
        val input = tx.inputsOfType<Obligation>().single()
        val output = tx.outputsOfType<Obligation>().single()
        "Only the defaulted property may change." using (input.withoutDefaulted() == output.withoutDefaulted())
        "The default property must be set to true on output." using (output.defaulted == true)
        "The default property must not have been true on input." using (input.defaulted != true)
        "The borrower and lender only must sign an obligation default transaction" using
                (signers == keysFromParticipants(input))
    }
}