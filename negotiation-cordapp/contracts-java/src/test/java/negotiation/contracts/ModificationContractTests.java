...
public class ModificationContractTests {
    ...
    @Test
    public void proposalModificationTransactionsHaveOnlyOneInputAndOneOutputState(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                tx.command(ImmutableList.of(alice.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
                tx.tweak(tdsl->{
    ...
    @Test
    public void proposalModificationTransactionsHaveInputOfTypeProposalStateAndOutputOfTypeProposalState(){
        ledger(ledgerServices, (ledger -> {
            ledger.transaction(tx -> {
                tx.command(ImmutableList.of(alice.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
                tx.tweak(tdsl ->{
    ...
    @Test
    public void inputProposalStateAndOutputProposalStateShouldHaveDifferentAmounts(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                tx.command(ImmutableList.of(alice.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
                tx.tweak(tdsl ->{
    ...
    public void proposalAcceptanceTransactionsHaveTwoRequiredSignersTheProposerAndProposee(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                ...
                tx.output(ProposalAndTradeContract.ID, new ProposalState(2, alice.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
                tx.tweak(tdsl ->{
                    tdsl.command(ImmutableList.of(alice.getPublicKey(), charlie.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
                    tdsl.fails();
                    return null;
                });
                tx.tweak(tdsl -> {
                    tdsl.command(ImmutableList.of(charlie.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
                    tdsl.fails();
                    return null;
                });
                tx.command(ImmutableList.of(alice.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
    ...
    public void buyerAndSellerAreUnmodifiedInTheOutput(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                ...
                tx.command(ImmutableList.of(alice.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Modify());
                tx.tweak(tdsl ->{
                    tdsl.output(ProposalAndTradeContract.ID, new ProposalState(1, alice.getParty(), charlie.getParty(), alice.getParty(), bob.getParty()));
                    tdsl.fails();
                    return null;
                });
                tx.tweak(tdsl ->{
                ...
                tx.tweak(tdsl ->{
                    tdsl.output(ProposalAndTradeContract.ID, new ProposalState(2, bob.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
                    tdsl.fails();
                    return null;
                });
                tx.verifies();
    ...
    @Test
    public void proposalModificationTransactionsHaveTwoRequiredSignersTheProposerAndProposee(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                tx.input(ProposalAndTradeContract.ID, new ProposalState(1, alice.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
                tx.output(ProposalAndTradeContract.ID, new ProposalState(2, alice.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
